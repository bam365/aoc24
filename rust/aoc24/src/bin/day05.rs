...
mod parsers {
    use nom::{Parser, IResult, bytes::complete::tag, character::complete::*, multi::*, sequence::*};
    use super::{Rule, Update, Problem};

    fn rule(input: &str) -> IResult<&str, Rule> {
        separated_pair(u32, tag("|"), u32)
            .map(|v| Rule { before: v.0, after: v.1 })
            .parse(input)
    }
    ...
    pub fn problem<'a>(input: &'a str) -> IResult<&'a str, Problem> {
        ...
        separated_pair(rules, pair(newline, newline), updates)
            .map(|v| Problem { 
                rules: v.0,
                updates: v.1.iter().filter(|v| v.pages.is_empty() == false).cloned().collect()
            })
            .parse(input)
    }
...
fn main() {
    ...
    let answer: u32 = input.updates.iter()
        .filter_map(|u| if u.is_ordered(&input.rules) { Some(u.middle()) } else { None })
        .sum();
    println!("{answer}");
}
